function [h,d,pC] = GT(E1,E2,nu1,nu2,eta,beta,sigma,sk,ku,m2,m4)
...
%   beta     asperity radius of curvature
...
for i = 1:length(h)
    ...
    w_e = @(z) (z-d(i)).^2.5 .* verteilung_asp(z);
    
    pC(i) = 16/15 * sqrt(2) * pi * (eta * beta * sigma)^2 * E * sqrt(sigma/beta) * integral(w_e,d(i),Inf);
